...
import sys
import requests
from compute_correction_factors import calc_fpi_corr, get_cdf_filenames
...
log_filename = 'corr_facs_log.txt'
logging.basicConfig(
    level=logging.INFO,
    format='%(message)s',
    handlers=[
...
with mp.Pool(num_workers) as pool:
    results = pool.starmap(calc_fpi_corr, [(filename, out_path, [i, nfiles]) for i, filename in enumerate(filenames[:])], chunksize=32)
