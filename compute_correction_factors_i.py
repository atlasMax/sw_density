...
def _read_data(cdf_filename):
    ...
    if len(ni_fpi) == 0 or len(ne_fit) == 0 or len(vsc) == 0:
        return None
    else:
        return ne_fit, ni_fpi, t_e, vsc, N0, beta, ic, tint, sw_mode
...
def _write_to_csv(folderpath, output):
    ...
    filename = 'density_correction_stats_i.csv'
...
def calc_fpi_corr_i(cdf_filename, outpath=None, progress_track=None):
    logstatus = ''
    ...
    if data_result is None:
        ...
        ne_fit, ni_fpi, t_e, vsc, N0, beta, ic, tint, sw_mode = data_result
    ...
    for window_tint in window_tints:
        ...
        if np.isnan(ne_fit_mean) or np.isnan(ni_fpi_mean):
            c0 = np.nan
            _print_output(logstatus+'C')
            return
        else:
            c0 = ne_fit_mean / ni_fpi_mean
...
if __name__ == "__main__":
    cdf_filename='varg'
    calc_fpi_corr_i(cdf_filename, outpath='output_data/stats/')
