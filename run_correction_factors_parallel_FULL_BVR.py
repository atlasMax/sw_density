...
import sys
import requests
from compute_correction_factors_FULL_BVR import calc_fpi_corr_full, get_cdf_filenames
...
if __name__ == "__main__":
    ...
    with mp.Pool(num_workers) as pool:
        results = pool.starmap(calc_fpi_corr_full, [(filename, out_path, [i, nfiles]) for i, filename in enumerate(filenames[:])], chunksize=96)
