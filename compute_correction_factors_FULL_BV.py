...
def _read_data(cdf_filename):
    ...
    if len(ne_fpi) == 0 or len(ne_fit) == 0 or len(vsc) == 0:
        ...
        return ne_fit, ne_fpi, ni_fpi, vex_fpi, vey_fpi, vez_fpi, vix_fpi, viy_fpi, viz_fpi, t_e, t_i, bx, by, bz, vsc, N0, beta, ic, tint, sw_mode
...
def _write_to_csv(folderpath, output_list):
    ...
    filename = 'density_correction_stats_full_BV_0513.csv'
    
    # Ensure file exists and write header if not present
    os.makedirs(folderpath, exist_ok=True)
    if not os.path.exists(folderpath+filename):
        output_header = [
            'start', 'end', 'ce', 'ci', 'ne_fit', 'ne_fpi', 'ni_fpi', 'vex_fpi', 'vey_fpi', 'vez_fpi', 'vix_fpi', 'viy_fpi', 'viz_fpi', 't_e', 't_i', 'bx', 'by', 'bz', 'vsc', 'N0', 'beta', 'ic', 'tint', 'sw_mode'
        ]
...
def calc_fpi_corr_full(cdf_filename, outpath=None, progress_track=None):
    ...
    if data_result is None:
      ...
      ne_fit, ne_fpi, ni_fpi, vex_fpi, vey_fpi, vez_fpi, vix_fpi, viy_fpi, viz_fpi, t_e, t_i, bx, by, bz, vsc, N0, beta, ic, tint, sw_mode = data_result
